...
where
  "tick_port_overlay_rel depth irq_mask c c0 \<longleftrightarrow>
     c = tick_port_overlay depth irq_mask c0"

lemma tick_port_overlay_selectors [simp]:
  "Scheduler_V611_Parse.globals.eal6_port_critical_depth_'
      (tick_port_overlay depth irq_mask c) = depth"
  "Scheduler_V611_Parse.globals.eal6_port_interrupts_disabled_'
      (tick_port_overlay depth irq_mask c) = irq_mask"
...
lemma tick_port_overlay_idempotent [simp]:
  "tick_port_overlay depth irq_mask (tick_port_overlay d m c) =
   tick_port_overlay depth irq_mask c"
...
lemma tick_port_overlay_rel_post_state_iff:
  "rel_post_state
      (rel_prod (=) (tick_port_overlay_rel depth irq_mask)) X Y
   \<longleftrightarrow>
   X = map_post_state
     (\<lambda>(r, s). (r, tick_port_overlay depth irq_mask s)) Y"
  by (cases X; cases Y;
      force simp: rel_post_state.simps rel_set_def
        tick_port_overlay_rel_def)

lemma tick_port_overlay_rel_spec_iff_run:
  fixes p ::
    "('e::default, 'a, Scheduler_V611_Parse.globals) spec_monad"
  shows
    "rel_spec p p (tick_port_overlay depth irq_mask c) c
       (rel_prod (=) (tick_port_overlay_rel depth irq_mask))
     \<longleftrightarrow>
     run p (tick_port_overlay depth irq_mask c) =
       map_post_state
         (\<lambda>(r, s). (r, tick_port_overlay depth irq_mask s))
         (run p c)"
...
where
  "tick_port_overlay_bisim depth irq_mask p \<longleftrightarrow>
     rel_spec_monad (tick_port_overlay_rel depth irq_mask) (=) p p"

lemma tick_port_overlay_bisim_iff_run:
  "tick_port_overlay_bisim depth irq_mask p
   \<longleftrightarrow>
   (\<forall>c. run p (tick_port_overlay depth irq_mask c) =
      map_post_state
        (\<lambda>(r, s). (r, tick_port_overlay depth irq_mask s))
        (run p c))"
proof (rule iffI)
  assume bisim: "tick_port_overlay_bisim depth irq_mask p"
  have bisim':
    "rel_spec_monad (tick_port_overlay_rel depth irq_mask) (=) p p"
    using bisim by (simp add: tick_port_overlay_bisim_def)
  show
    "\<forall>c. run p (tick_port_overlay depth irq_mask c) =
       map_post_state
         (\<lambda>(r, s). (r, tick_port_overlay depth irq_mask s))
  ...
  proof
    ...
    have related:
      "rel_spec p p (tick_port_overlay depth irq_mask c) c
         (rel_prod (=) (tick_port_overlay_rel depth irq_mask))"
      by (rule rel_spec_monadD[OF bisim'])
         (simp add: tick_port_overlay_rel_def)
    then show
      "run p (tick_port_overlay depth irq_mask c) =
       map_post_state
         (\<lambda>(r, s). (r, tick_port_overlay depth irq_mask s))
         (run p c)"
      by (simp add: tick_port_overlay_rel_spec_iff_run)
  qed
next
  assume runs:
    "\<forall>c. run p (tick_port_overlay depth irq_mask c) =
       map_post_state
         (\<lambda>(r, s). (r, tick_port_overlay depth irq_mask s))
         (run p c)"
  show "tick_port_overlay_bisim depth irq_mask p"
    unfolding tick_port_overlay_bisim_def
  proof (rule rel_spec_monadI)
    fix s t
    assume related: "tick_port_overlay_rel depth irq_mask s t"
    then have s:
      "s = tick_port_overlay depth irq_mask t"
      by (simp add: tick_port_overlay_rel_def)
    show
      "rel_spec p p s t
         (rel_prod (=) (tick_port_overlay_rel depth irq_mask))"
      using runs[rule_format, of t]
      by (simp add: s tick_port_overlay_rel_spec_iff_run)
  qed
qed

lemma tick_port_overlay_bisim_yield:
  "tick_port_overlay_bisim depth irq_mask (yield x)"
  by (simp add: tick_port_overlay_bisim_iff_run)

lemma tick_port_overlay_bisim_gets:
  assumes invariant:
    "\<And>c. f (tick_port_overlay depth irq_mask c) = f c"
  shows "tick_port_overlay_bisim depth irq_mask (gets f)"
  using invariant
  by (simp add: tick_port_overlay_bisim_iff_run)

lemma tick_port_overlay_bisim_guard:
  assumes invariant:
    "\<And>c. P (tick_port_overlay depth irq_mask c) = P c"
  shows "tick_port_overlay_bisim depth irq_mask (guard P)"
  using invariant
...
lemma tick_port_overlay_bisim_modify:
  assumes commute:
    "\<And>c. f (tick_port_overlay depth irq_mask c) =
       tick_port_overlay depth irq_mask (f c)"
  shows "tick_port_overlay_bisim depth irq_mask (modify f)"
  using commute
  by (simp add: tick_port_overlay_bisim_iff_run)
...
lemma tick_port_overlay_bisim_condition:
  assumes invariant:
      "\<And>c. P (tick_port_overlay depth irq_mask c) = P c"
    and left: "tick_port_overlay_bisim depth irq_mask p"
    and right: "tick_port_overlay_bisim depth irq_mask q"
  shows
    "tick_port_overlay_bisim depth irq_mask (condition P p q)"
...
lemma tick_port_overlay_bisim_bind:
  fixes p ::
    "('e::default, 'a, Scheduler_V611_Parse.globals) spec_monad"
  assumes head: "tick_port_overlay_bisim depth irq_mask p"
    and tail: "\<And>x. tick_port_overlay_bisim depth irq_mask (k x)"
  shows "tick_port_overlay_bisim depth irq_mask (p >>= k)"
proof -
...
text \<open>
  ...
  @{term "tick_port_overlay_bisim depth irq_mask
