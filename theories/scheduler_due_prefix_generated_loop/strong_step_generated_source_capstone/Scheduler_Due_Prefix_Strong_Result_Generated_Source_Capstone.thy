theory Scheduler_Due_Prefix_Strong_Result_Generated_Source_Capstone
  imports
    "EAL6_FreeRTOS_V611_Scheduler_Due_Prefix_Strong_Result_State_Assembler.Scheduler_Due_Prefix_Strong_Result_State_Assembler"
    "EAL6_FreeRTOS_V611_Scheduler_Due_Prefix_Exact_Full_State_Propagation.Scheduler_Due_Prefix_Exact_Full_State_Propagation"
begin

theorem DueLoopStrongHeadRel_nonlast_result_full:
  assumes strong:
  ...
  shows
    "one_due_tick_loop_body_source (sd_tcb_ptr D task) \<bullet> c
     \<lbrace>DueLoopSharedResultPost D R now entry processed task u
...
proof -
  ...
  note source_C = due_prefix_generated_source_result_step_full_state[
    OF gate_C roots]
  have source:
    "one_due_tick_loop_body_source (sd_tcb_ptr D task) \<bullet> c
  ...
  have strong_after:
    ...
    by (rule DueLoopStrongHeadRel_nonlast_result_full_state[
  ...
  proof (rule runs_to_weaken[OF source])
    ...
    show
      ...
      unfolding DueLoopSharedResultPost_def Let_def
      apply (intro conjI)
...
text \<open>
  The public Result theorem is intentionally placed after the semantic family
  and scalar ledgers in this staircase.  Its required boundary is fixed here
  by DueLoopSharedResultPost: no future rung may weaken it by adding a desired
  post-family, successful execution, Event owner, or fixed runtime value to the
  assumptions.
\<close>
