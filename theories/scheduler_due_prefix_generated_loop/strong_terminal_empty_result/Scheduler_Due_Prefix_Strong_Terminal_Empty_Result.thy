theory Scheduler_Due_Prefix_Strong_Terminal_Empty_Result
  imports
...
text \<open>
  Exact terminal-Empty endpoint for one last due task.  The Generic family,
  Event family and phase snapshot are the single source-order witnesses
  computed from the entry snapshot.  The endpoint retains both the relaxed
  due-loop relation and its phase-aware stable terminal strengthening.
\<close>

definition DueLoopSharedLastEmptyEndpoint ::
...
theorem DueLoopStrongHeadRel_last_empty_result_full_state:
  assumes strong:
    ...
    and gate:
      "due_prefix_gate_inv D R c now entry processed
         [Generic task] [] current C branch S generic_raw event_raw"
...
proof -
  ...
  have core_before: "due_loop_core_wf current"
    and time_before:
      "due_loop_time_wf now [Generic task] [] current"
    and domain_before:
  ...
  have loop_before:
    "due_prefix_loop_inv now entry processed [Generic task] [] current"
  ...
  have terminal_after:
    "StrongDuePrefixLoopHeadRel D ?post_c ?after managed termination external
       ...
       now entry (processed @ [Generic task]) [] []
       EmptyExit None NULL"
    by (rule DueLoopStrongHeadRel_terminal_strong[OF strong_after])
  show ?thesis
    using strong_after terminal_after by (simp add: Let_def)
qed

theorem DueLoopStrongHeadRel_last_empty_result_full:
  assumes strong:
    "DueLoopStrongHeadRel D c current managed termination external
       generic_raw (ods_generic_family S)
       event_raw (ods_event_family S) K_G K_E S
  ...
  shows
    "one_due_tick_loop_body_source (sd_tcb_ptr D task) \<bullet> c
     \<lbrace>\<lambda>r t. r = Result NULL \<and>
       DueLoopSharedLastEmptyEndpoint D now entry processed task C branch S
         generic_raw event_raw K_G K_E managed termination external c t\<rbrace>"
proof -
  have gate_C:
    "due_prefix_gate_inv D R c now entry processed
       [Generic (odc_task C)] [] current C branch S generic_raw event_raw"
    using gate selector by simp
  have local:
    "one_due_gateH_entry_rel D R c current C branch S
       generic_raw event_raw"
    using gate_C by (simp add: due_prefix_gate_inv_def)
  note body = due_prefix_generated_last_due_empty_body[OF gate_C roots]
    ...
    "one_due_tick_loop_body_source (sd_tcb_ptr D task) \<bullet> c
     \<lbrace>\<lambda>r t.
       ...
       due_prefix_terminal_source_rel D []
         (due_prefix_result_step_abs entry processed (Generic task))
         EmptyExit None NULL\<rbrace>"
    using body selector by simp
    ...
    "one_due_tick_loop_body_source (sd_tcb_ptr D task) \<bullet> c
     \<lbrace>\<lambda>r t.
       ...
       t = one_due_tick_ready_inserted_state
         D C branch generic_raw c\<rbrace>"
    using body_task exact_state_task by (simp only: runs_to_conj)
  have state_strong:
    "let after =
         due_prefix_result_step_abs entry processed (Generic task);
         h0 = hrs_mem (Scheduler_V611_Parse.globals.t_hrs_' c);
     ...
     in DueLoopStrongHeadRel D post_c after managed termination external
        ...
        StrongDuePrefixLoopHeadRel D post_c after managed termination external
          ...
          now entry (processed @ [Generic task]) [] []
          EmptyExit None NULL"
    by (rule DueLoopStrongHeadRel_last_empty_result_full_state[
          OF strong gate selector roots])
  show ?thesis
  proof (rule runs_to_weaken[OF source])
    ...
    note terminal_rel = conjunct2[OF post_tail2]
    have terminal_post:
      "due_prefix_generated_terminal_post D now entry
    ...
    note state_pair = state_strong[unfolded Let_def]
    note due_strong = conjunct1[OF state_pair]
    note stable_strong = conjunct2[OF state_pair]
    show
      "r = Result NULL \<and>
       DueLoopSharedLastEmptyEndpoint D now entry processed task C branch S
         generic_raw event_raw K_G K_E managed termination external c t"
      unfolding DueLoopSharedLastEmptyEndpoint_def Let_def
      using r_eq state_eq body_post exit_post terminal_rel terminal_post
        due_strong stable_strong by simp
  qed
qed

theorem DueLoopStrongHeadRel_last_empty_bare_loop_full:
  assumes strong:
    "DueLoopStrongHeadRel D c current managed termination external
       generic_raw (ods_generic_family S)
       event_raw (ods_event_family S) K_G K_E S
...
proof -
  ...
  have head_nonnull: "sd_tcb_ptr D task \<noteq> NULL"
    using due_prefix_gate_inv_head_nonnull[OF gate_C] selector by simp
  note body = DueLoopStrongHeadRel_last_empty_result_full[
    OF strong gate selector roots]
  show ?thesis
  ...
  proof (rule runs_to_whileLoop_unroll_exn)
    show
      "\<not> (\<lambda>p _. p \<noteq> NULL) (sd_tcb_ptr D task) c \<Longrightarrow>
       ...
       DueLoopSharedLastEmptyEndpoint D now entry processed task C branch S
         generic_raw event_raw K_G K_E managed termination external c c"
         ...
         (\<forall>b. r = Result b \<longrightarrow>
           whileLoop (\<lambda>p _. p \<noteq> NULL)
             ...
             \<lbrace>\<lambda>q u. q = Result NULL \<and>
               DueLoopSharedLastEmptyEndpoint D now entry processed task
                 C branch S generic_raw event_raw K_G K_E managed
      ...
      proof (rule runs_to_weaken[OF body])
        ...
        have loop_null:
          "whileLoop (\<lambda>p _. p \<noteq> NULL)
           ...
           \<lbrace>\<lambda>q u. q = Result NULL \<and>
             DueLoopSharedLastEmptyEndpoint D now entry processed task C branch S
               generic_raw event_raw K_G K_E managed termination external
               c u\<rbrace>"
...
theorem DueLoopStrongHeadRel_last_empty_finally_full:
  assumes strong:
    "DueLoopStrongHeadRel D c current managed termination external
       generic_raw (ods_generic_family S)
       event_raw (ods_event_family S) K_G K_E S
...
proof -
  ...
  proof (rule runs_to_weaken[OF
      DueLoopStrongHeadRel_last_empty_bare_loop_full[
        OF strong gate selector roots]])
    fix r ::
      "(unit, Scheduler_V611_Parse.tskTaskControlBlock_C ptr) xval"
    fix t :: Scheduler_V611_Parse.globals
  ...
  qed
  show ?thesis
    by (rule due_prefix_generated_finally_normalises[OF bare])
qed

text \<open>
  Quantifier audit: task, priority, tick, roots, raw rings, cursors, payloads,
  managed/termination/external populations, heap layout, branch and processed
  prefix all remain symbolic.  The only specialised structure is the semantic
  branch proved here: exactly one due item remains and the future suffix is
  empty.  EmptyExit, NULL and Result () are generated control outcomes, not
