theory Scheduler_Due_Prefix_Managed_Gate_Terminal_State_Core
  imports
    "EAL6_FreeRTOS_V611_Scheduler_Due_Prefix_Managed_Gate_Nonlast_Capstone.Scheduler_Due_Prefix_Managed_Gate_Nonlast_Capstone"
begin

text \<open>
  Internal result-state assembler for every symbolic tail.  The public Empty
  and Future rungs below discharge the sole control-local parameter
  post_pxTCB from the exact terminal shape; it is not an assumed
  concrete poststate.
\<close>

theorem DueLoopStrongHeadRel_managed_gate_result_full_state_core:
  assumes strong:
...
proof -
  ...
  have task_live: "task \<in> sa_live current"
    using core_before task_delayed
  ...
  have families:
    ...
    by (rule DueLoopStrongHeadRel_managed_gate_full_family_cutpoints[
          OF strong gate selector roots])
  ...
  have generic_role_after:
    "strong_generic_role_projection ?after termination
       (ods_generic_family ?S')"
    by (rule DueLoopStrongHeadRel_managed_gate_generic_role_projection[
          OF strong gate selector roots])
  have waiting_after:
...
theorem DueLoopStrongHeadRel_managed_gate_last_empty_full_state:
  assumes strong:
    "DueLoopStrongHeadRel D c current managed termination external
       generic_raw (ods_generic_family S)
       event_raw (ods_event_family S) K_G K_E S
...
proof -
  ...
  note due = DueLoopStrongHeadRel_managed_gate_result_full_state_core[
  ...
  have stable:
    "let after = due_prefix_result_step_abs entry processed (Generic task);
         ...
         S' = one_due_reentry_snapshot C branch S;
         post_c = one_due_tick_ready_inserted_state D C branch generic_raw c
     in StrongDuePrefixLoopHeadRel D post_c after managed termination external
          generic_raw' (ods_generic_family S')
          event_raw' (ods_event_family S') K_G K_E S'
          now entry (processed @ [Generic task]) [] []
          EmptyExit None NULL"
    using DueLoopStrongHeadRel_terminal_strong[OF due0]
    by (simp add: Let_def)
  show ?thesis using due stable by (simp add: Let_def)
qed

definition DueLoopManagedStrongTerminalFutureState ::
...
lemma StrongDuePrefixLoopHeadRel_managed_terminal_future_ready:
  assumes head:
...
proof -
  ...
  have member: "Generic f \<in> set (ring (current_delayed_ring current))"
    using ring by simp
  ...
  have physical_key:
    ...
    using StrongSchedulerSnapshotRel_generic_payloadD[OF strong managed_task]
...
theorem DueLoopStrongHeadRel_managed_gate_last_future_full_state:
  assumes strong:
    "DueLoopStrongHeadRel D c current managed termination external
       generic_raw (ods_generic_family S)
       event_raw (ods_event_family S) K_G K_E S
...
proof -
  ...
  have ready:
    ...
    by (rule StrongDuePrefixLoopHeadRel_managed_terminal_future_ready[
          OF terminal0])
  show ?thesis
    unfolding DueLoopManagedStrongTerminalFutureState_def Let_def
