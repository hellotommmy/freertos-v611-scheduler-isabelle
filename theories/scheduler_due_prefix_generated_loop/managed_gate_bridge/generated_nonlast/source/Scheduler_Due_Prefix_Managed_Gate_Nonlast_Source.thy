theory Scheduler_Due_Prefix_Managed_Gate_Nonlast_Source
  imports
    "EAL6_FreeRTOS_V611_Scheduler_Due_Prefix_Managed_Gate_Capstone.Scheduler_Due_Prefix_Managed_Gate_Capstone"
begin

text \<open>
  The generated body is driven by Gate-H over the allocated managed view,
  while the due-prefix fold remains the real runnable scheduler state.  The
  source theorem below is a narrow reuse of one_due_tick_gate_step_full_state:
  it does not reopen the generated VCG and does not assume any successor
  branch, heap, family or abstract poststate.
\<close>

theorem due_prefix_managed_generated_nonlast_source_full_state:
  assumes inv:
...
proof -
  ...
  have source_step:
    "one_due_tick_loop_body_source (sd_tcb_ptr D (odc_task C)) \<bullet> c
     \<lbrace>\<lambda>r t.
       ...
       t = one_due_tick_ready_inserted_state
         D C branch generic_raw c\<rbrace>"
    by (rule one_due_tick_gate_step_full_state[
  ...
  proof (rule runs_to_weaken[OF source_step])
    ...
    let ?after =
      "due_prefix_result_step_abs entry processed
        (Generic (odc_task C))"
    have managed_live_frame:
      "sa_live (managed_scheduler_view ?after managed) =
       sa_live (managed_scheduler_view current managed)"
      by (simp add: managed_scheduler_view_def)
    have managed_priority_frame:
      "sa_priority (managed_scheduler_view ?after managed) =
       sa_priority (managed_scheduler_view current managed)"
      using current_def
      by (simp add: managed_scheduler_view_def)
    have gate_new:
      "one_due_gateH_entry_rel D R t
      ...
      by (rule one_due_gateH_entry_rel_abs_param_cong[
            OF gate_old managed_live_frame managed_priority_frame])
    show
      "\<exists>branch'.
        r = Result (sd_tcb_ptr D u) \<and>
        due_prefix_managed_gate_inv D R t now entry
        ...
        t = one_due_tick_ready_inserted_state
          D C branch generic_raw c"
    proof (rule exI[of _ branch'], intro conjI)
      show "r = Result (sd_tcb_ptr D u)" by (rule result)
      show
        "due_prefix_managed_gate_inv D R t now entry
          (processed @ [Generic (odc_task C)])
          (Generic u # remaining) future ?after managed
          ...
          (one_due_reentry_generic_raw D C
            ...
            generic_raw)
          (one_due_event_raw_after_remove D C branch event_raw)"
        unfolding due_prefix_managed_gate_inv_def
